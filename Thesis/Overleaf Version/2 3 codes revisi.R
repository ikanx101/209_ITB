...
milp_new = 
  ...
  # week 1
  #add_variable(tot_w1[k],type = "continuous",lb = 0,k = G) %>%
  #add_constraint(tot_w1[k] == 0.5 * ic * sum_expr(z[j,k] + Z_0k[k] + x_hat_stb[j,k],
  #                                           j = 1),
  #               k = G) %>% 
  # week 2-4
  ...
  # objective function
  set_objective(sum_expr(c_k[k] * x_stb[k], #+ tot_w1[k] + tot_w24[k] - yearly[k]
                         k = G),
                "min")
