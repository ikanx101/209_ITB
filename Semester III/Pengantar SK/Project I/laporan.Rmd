...
Zat A akan luruh dengan laju sebesar $r_A$ membentuk zat B dengan laju pertumbuhan sebesar $r_A$ juga. Kemudian zat B akan luruh dengan laju sebesar $r_B$ membentuk zat C dengan laju pertumbuhan sebesar $r_B$ juga. 
...
1. Perubahan massa zat C bergantung pada pembentukan saja.
...
```{r,echo=FALSE}
data.frame(Parameter = c("r_a","r_b","qa0","qb0","qc0","dt0","iter_length"),
           Value = c(.5,.4,10,0,0,.25,5)) %>% knitr::kable(caption = "Parameter Simulasi Program Euler")
```

```{r out.height="50%",echo=FALSE,fig.cap = "Hasil Simulasi Program Euler",fig.align='center'}
...
# definisi fungsi persamaan diferensial
  # zat radio aktif A
  d_a = function(t,q_a){(-r_a * q_a)}
  
  # zat radio aktif B
  d_b = function(t,q_a,q_b){((r_a * q_a) - (r_b * q_b))}
  
  # zat radio aktif C
  d_c = function(t,q_b,q_c){(r_b * q_b)}
...
for(i in 2:num_iter){
  ...
  # kita hitung zat b
  k1 = d_b(t,A[i-1],B[i-1]) 
  k2 = d_b(t + 0.5*h,A[i-1] + 0.5*k1*h,B[i-1] + 0.5*k1*h) 
  k3 = d_b(t + 0.5*h,A[i-1] + 0.5*k2*h,B[i-1] + 0.5*k2*h) 
  k4 = d_b(t + h,A[i-1] + k3*h,B[i-1] + k3*h) 
  
  B[i] = B[i-1] + (1/6)*(k1 + 2*k2 + 2*k3 + k4) * h
  
  # kita hitung zat c
  k1 = d_c(t,B[i-1],C[i-1]) 
  k2 = d_c(t + 0.5*h,B[i-1] + 0.5*k1*h,C[i-1] + 0.5*k1*h) 
  k3 = d_c(t + 0.5*h,B[i-1] + 0.5*k2*h,C[i-1] + 0.5*k2*h) 
  k4 = d_c(t + h,B[i-1] + k3*h,C[i-1] + k3*h) 
...
Sekarang saya akan _run_ program Euler dan Runge Kutta $4^{th}$ menggunakan parameter yang ada di soal, yakni:

```{r,echo=FALSE}
data.frame(Parameter = c("r_a","r_b","qa0","qb0","qc0","dt0","iter_length"),
           Value = c(.0137,.051,4*10^(-8),0,0,.5,10)) %>% 
           knitr::kable(caption = "Parameter di Soal")
```

Massa zat A adalah $S \times 10^{-8}$ dengan $S$ adalah 2 digit terakhir NIM saya (20921004). Sehingga $S = 4$.

## Menggunakan Program Euler

### Tabel Simulasi

Berikut ini adalah hasil simulasi menggunakan program Euler:
...
```{r,echo=FALSE,fig.cap = "Ilustrasi Hasil Simulasi Euler Menggunakan Parameter Soal"}
rm(list=ls())
...
df = data.frame(t,A = q_a,B = q_b,C = q_c)
...
Terlihat bahwa zat A luruh sepenuhnya membentuk zat B yang kemudian luruh sepenuhnya sehingga hanya menyisakan zat C yang terus bertumbuh hingga massanya sebesar $Q_A(t = 0) = 4 \times 10^{-8}$ gram di akhir waktu.

## Menggunakan Program Runge Kutta $4^{th}$

### Tabel Hasil Simulasi

Berikut ini adalah hasil simulasi menggunakan program Runge Kutta $4^{th}$:
...
Berikut ini adalah grafiknya jika saya perpanjang nilai $t$-nya menjadi 300 dan $h = 0.05$ :

```{r,echo=FALSE,fig.cap = "Ilustrasi Hasil Simulasi RK4 Menggunakan Parameter Soal"}
rm(list=ls())
...
df %>% 
  ...
  scale_color_manual(values = c("darkred","steelblue","green")) +
  labs(title = "Hasil Simulasi dengan Parameter dari Soal",
       subtitle = "Menggunakan Program Runge Kutta 4th",
       y = "Massa zat",
       color = "Jenis Zat Radioaktif",
       caption = "20921004 - Mohammad Rizka Fadhli")
...
Terlihat bahwa grafik yang dihasilkan program Runge Kutta $4^{th}$ sama. Hasilnya adalah zat A luruh sepenuhnya membentuk zat B yang kemudian luruh sepenuhnya sehingga hanya menyisakan zat C yang terus bertumbuh hingga massanya sebesar $Q_A(t = 0) = 4 \times 10^{-8}$ gram di akhir waktu.

## Nilai Maksimum Massa Zat B

Jika kita lihat pada grafik sebelumnya, nilai massa zat B akan maksimal di suatu waktu $t$ tertentu. Kita akan coba aproksimasi nilainya.

Secara analitik, nilai maksimum terjadi saat $\frac{dB}{dt} = 0$.

$$\frac{dB}{dt} = r_A A - r_B B = 0$$

$$r_A A = r_B B$$

$$0.0137 A = 0.051 B$$

$$\frac{A}{B} = \frac{0.051}{0.0137} = 3.722628$$

B akan mencapai maksimum saat rasio $\frac{A}{B}$ di suatu $t$ tertentu bernilai $3.722628$. Oleh karena itu, saya akan _run_ ulang __Program RK4.R__ dengan parameter $h$ dibuat kecil ($h = 0.0001$). Kemudian saya akan cari $t$ tertentu saat rasio $\frac{A}{B}$ sesuai dengan perhitungan analitik di atas.
...
# output
output = data.frame(t_,A,B,C)
output %>% filter(B == max(B))

```

Saya dapatkan massa B maksimum adalah sekitar $B = 6.630437 \times 10^{-9}$ gram pada $t = 35.2394$.

Rasio $\frac{A}{B} =$ `r 2.468263e-08 / 6.630437e-09` hampir sama dengan rasio hasil analitik.


## Perubahan Laju Peluruhan $r_A$ dan $r_B$

Apa yang akan terjadi jika:

### $r_A < r_B$

Kasus ini sama persis dengan apa yang sudah disimulasikan pada bagian sebelumnya. Hal yang akan terjadi adalah:

- Peluruhan zat A akan relatif lebih lama sehingga pembentukan zat B juga akan sama lamanya.
- Namun zat B akan lebih cepat luruh dibandingkan terbentuk.
- Akibatnya:
    - C akan lebih cepat terbentuk.
    - B akan lebih cepat mencapai maksimum sebelum A luruh sepenuhnya. 
    - B akan lebih cepat luruh sebelum A luruh.

Berikut ini adalah beberapa simulasi lain menggunakan parameter $r_A$ dan $r_B$ yang berbeda-beda.


```{r,echo=FALSE,fig.cap = "Ilustrasi Hasil Simulasi Saat rA < rB (1)"}
rm(list=ls())

# definisi dan initial condition
r_a = .01  # rate peluruhan A dan pertumbuhan B
...
df %>% 
  ...
  geom_line() +
  scale_color_manual(values = c("darkred","steelblue","green")) +
  labs(title = "Hasil Simulasi dengan rA < rB",
       subtitle = "rA = 0.01 dan rB = 0.05",
       y = "Massa zat",
       color = "Jenis Zat Radioaktif",
       caption = "20921004 - Mohammad Rizka Fadhli")
```
...
```{r,echo=FALSE,fig.cap = "Ilustrasi Hasil Simulasi Saat rA < rB (2)"}
rm(list=ls())

# definisi dan initial condition
r_a = .3  # rate peluruhan A dan pertumbuhan B
r_b = .7  # rate peluruhan B dan pertumbuhan C
r_c = .0  # rate peluruhan C
...
Jika kondisi yang terjadi adalah $r_A > r_B$, maka:

- Peluruhan zat A akan relatif lebih cepat sehingga pembentukan zat B juga akan sama cepatnya.
- Zat B akan lebih cepat terbentuk dibandingkan meluruh.
- Akibatnya:
    - C akan lebih lambat terbentuk.
    - B akan lebih cepat mencapai maksimum bersamaan dengan A luruh sepenuhnya. 
    - A akan lebih cepat luruh sebelum B luruh.
...
# definisi dan initial condition
r_a = .09  # rate peluruhan A dan pertumbuhan B
r_b = .02  # rate peluruhan B dan pertumbuhan C
r_c = .0  # rate peluruhan C
...
- Masalah persamaan diferensial bisa diselesaikan dengan menggunakan metode aproksimasi diskrit seperti algoritma Euler dan Runge Kutta $4^{th}$ _order_. Hasil kedua algoritma tersebut bisa mendekati hasil analitik jika kita perkecil $\Delta t$ yang digunakan pada saat kita _run_ program tersebut.
- Dinamika model akan bisa terlihat saat kita mengubah-ubah nilai laju peluruhan zat ($r_A$ dan $r_B$).
