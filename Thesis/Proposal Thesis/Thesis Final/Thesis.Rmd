...
title: "T E S I S"
author: "Mohammad Rizka Fadhli<br>Dr. Saladin Uttunggadewa<br>Dr. Rieske Hadianti<br>Prof. Dr. Sri Redjeki<br>Astri Kurniati, ST., M.App.Sc."
...
# https://garthtarr.github.io/sydney_xaringan/#48
```
...
## Penjelasan Mengenai Luaran pada Tesis
...
# _QUICK RECAP_: Presentasi Proposal Tesis
...
Untuk pemenuhan bahan baku tersebut, NFI menggunakan prinsip _multiple sourcing_ dengan perjanjian untuk memasoknya dari enam buah _supplier_.

> Catatan: __Satu supplier memasok satu jenis bahan baku__.

--

Perjanjian kerja sama antar _supplier_ berupa komitmen pembelian sejumlah bahan baku selama setahun ke depan. Spesifikasi bahan baku dan harganya per-unit berbeda-beda antar _supplier_. 
...
Jadi walau beda-beda rasa dan warna, __tapi__ ada satu bahan baku utama.

Jelasin bahwa satu _supplier_ itu memasok hanya satu jenis bahan baku saja.

Lalu jelasin kenapa hanya bisa diproduksi oleh satu jenis bahan baku saja? Kenapa diperbolehkan dua atau lebih?

> Perbedaan mendasar antar bahan baku ini adalah pada warnanya. Untuk produk dengan warna-warna muda, hanya bisa oleh bahan baku warna muda. Kalau gelap, bisa semua.
...
Jelasin dulu siapa itu PPIC alias _production planning and inventory control_
...
1. Pembelian bahan baku dari _supplier_ harus memenuhi proporsi portofolio yang ditetapkan di awal tahun oleh __NFI__. Angka ini kelak menjadi patokan berapa kuantitas minimal yang harus __NFI__ beli kepada masing-masing _supplier_ dalam setahun.
...
1. Fungsi _objective_ dan _constraints_ masih berupa _linear_.
    - Meminimumkan ongkos pembelian `harga x kuantitas pembelian`.
...
Data-data ini diperoleh dari NFI.

sebagai contoh $f_{ik}$ didapatkan dari __resep produksi__.

Sementara $g_{ijk}$ itu diturunkan dari data resep produksi dan demand mingguan bahan baku per item.
...
Misal suatu produk tidak dipilih, maka $a_{ijk}=0$ walau $f_{ik} = 1$ akibatnya $b_{ijk} = 0$ yang bawahnya terpenuhi

Misal suatu produk dipilih, maka $a_{ijk}=1$ misal $f_{ik} = 1$ akibatnya $b_{ijk} \geq 0$ maka diperlukan $\mu$ agar terpenuhi
...
Kita pakai rumus luas trapesium

$\frac{1}{2} (alas_1 + alas_2) \times tinggi$
...
Masih ada yang belum 100% otomatis seperti pada penemuan error akibat tidak ada solusi optimal.
...
### Pengembangan DSS Lebih Lanjut

Mengawinkan antara skrip optimisasi dengan _shinyapps_ memiliki tantangan tersendiri karena sumber daya komputasi yang dibutuhkan menjadi sangat besar. Oleh karena itu bisa dicoba untuk meng-_contain_ _computing environment_ termasuk _compiler_ menggunakan `DOCKER`, lalu __menempelkannya__ ke _high performance computer_.
