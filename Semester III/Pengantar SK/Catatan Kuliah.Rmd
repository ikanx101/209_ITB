...
_Link_ penting untuk menyelesaikan persamaan diferensial:
...
Metode _Euler_ sangat simpel, yakni dengan memanfaatkan persamaan beda dengan $\Delta t$ yang kecil sehingga $P_t$ diaproksimasi dengan gradien garis yang ada.

### Algoritma _Revised Euler_
...
Initialize 
    ...
    NumberOfSteps
Loop
    for n from 1 to NumberOfSteps
        tn = t0 + n dt
        Pn = Pn-1 + f(tn-1,Pn-1) dt
```

### Contoh Soal

$$\frac{dP}{dt} = 10 + \frac{P}{3}$$

dengan $P_0 = 300$ dan $\Delta t = 0.1$, hitung:

a. $\frac{dP}{dt} saat $t = 0$
b. $P_1$
c. $P_2$
...
Solusi analitik dari permasalahan di atas adalah:

$$P\left(x\right)=-e^{\frac{-x-c_1}{3}}+30$$

## Metode _Runge Kutta_ `2`

### Algoritma _Runge Kutta_ `2`
...
Loop
    for n from 1 to NumberOfSteps
        tn = t0 + n dt
        Yn = Pn-1 + f(tn-1,Pn-1) dt
        Pn = Pn-1 + 0.5 (f(tn-1,Pn-1) + f(tn,Yn)) dt
```
...
## _Runge Kutta_ Orde 4

Ini function di R yang dulu pernah bikin untuk kuliah algoritma dan perancangan software.

```{r}
rk_4order = function(f, x0, y0, dt, n){
  # initial condition
  x = x0
  y = y0
  # proses iterasi
