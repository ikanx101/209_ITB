...
Pada materi-materi awal, _desk research_ merupakan salah satu metode _market research_ yang mengandalkan data sekunder. Kita bisa mengambil data dari sumber-sumber terpercaya dan melakukan analisa langsung tanpa harus melakukan survey terlebih dahulu.
...
Salah satu contohnya adalah _proce analysis_ dari pasar mobil bekas di area Jabodetabek. Sebagaimana yang telah kita ketahui bersama, ada berbagai portal _online_ dalam jual-beli mobil bekas. Salah satu yang terkenal adalah situs [Carmudi](https://www.carmudi.co.id/en).

Kali ini kita akan lakukan _web scraping_, yakni mengambil data publik yang tersedia di _web_ dengan menggunakan algoritma yang ditulis menggunakan bahasa pemrograman __R__. Kita akan ambil data-data sebagai berikut:
...
Berikut adalah data yang bisa kita dapatkan dari situs tersebut:

```{r,include=FALSE}
rm(list=ls())
load("data_carmudi.rda")
data_all %>% knitr::kable(align = "c")
```

### Analisa Harga per _Brand_

Kita bisa melakukan analisa harga mobil per _brand_ sebagai berikut:

```{r,echo=FALSE,message=FALSE,warning=FALSE}
data_clean = 
  data_all %>% 
  mutate(brand = case_when(
    grepl("suzuki",nama,ignore.case = T) ~ "Suzuki",
    grepl("honda",nama,ignore.case = T) ~ "Honda",
    grepl("daihatsu",nama,ignore.case = T) ~ "Daihatsu",
    grepl("isuzu",nama,ignore.case = T) ~ "Isuzu",
    grepl("toyota",nama,ignore.case = T) ~ "Toyota",
    grepl("mercedes",nama,ignore.case = T) ~ "Mercedes",
    grepl("bmw",nama,ignore.case = T) ~ "BMW",
    grepl("Mazda",nama,ignore.case = T) ~ "Mazda",
    grepl("Mitsubishi",nama,ignore.case = T) ~ "Mitsubishi",
    grepl("Ford",nama,ignore.case = T) ~ "Ford",
    grepl("Nissan",nama,ignore.case = T) ~ "Nissan",
    grepl("Porsche",nama,ignore.case = T) ~ "Porsche",
    grepl("Hyundai",nama,ignore.case = T) ~ "Hyundai",
    grepl("Lexus",nama,ignore.case = T) ~ "Lexus",
    grepl("Subaru",nama,ignore.case = T) ~ "Subaru",
    grepl("Volkswagen",nama,ignore.case = T) ~ "Volkswagen"
  )) 

data_clean %>% 
  group_by(brand) %>% 
  summarise(max = max(harga)/1000000,
            med = median(harga)/1000000,
            min = min(harga)/1000000) %>% 
  ungroup() %>% 
  ggplot(aes(x = brand)) +
  ...
  labs(title = "Sebaran Harga Mobil Bekas dari Situs Carmudi per Brand",
       subtitle = "sumber: situs Carmudi Pada Januari 2022",
       x = "Merek Mobil",
...
data_clean %>% 
  ...
  summarise(max = max(harga)/1000000,
            ...
            min = min(harga)/1000000) %>% 
  ungroup() %>% 
  ggplot(aes(x = factor(tahun))) +
  geom_errorbar(aes(ymin = min,
                    ymax = max),
                width = .3) +
  geom_point(aes(y = med)) +
  labs(title = "Sebaran Harga Mobil Bekas dari Situs Carmudi per Tahun",
       subtitle = "sumber: situs Carmudi Pada Januari 2022",
       x = "Tahun Mobil",
       y = "Harga (dalam juta rupiah)") +
  theme_minimal() +
...
data_clean %>% 
  ...
  geom_errorbar(aes(ymin = min,
                ...
                width = .3) +
  geom_point(aes(y = med)) +
  labs(title = "Sebaran Harga Mobil Bekas dari Situs Carmudi per Brand per Tahun",
       subtitle = "sumber: situs Carmudi Pada Januari 2022",
       x = "Tahun Mobil",
       y = "Harga (dalam juta rupiah)") +
  theme_minimal() +
  facet_wrap(~brand,scales = "free",ncol = 3) +
  theme(axis.text.x = element_text(face = "bold",angle = 90),
        axis.text.y = element_text(face = "bold",size = 10),
...
Pada pertemuan ini, saya akan memberitahukan beberapa contoh _market research projects_ yang biasa dilakukan oleh industri:

## _Retail Audit_

Salah satu contoh _market research project_ yang erat kaitannya dengan dunia _sales_ adalah _retail audit_. _Retail audit_ berarti mengumpulkan data transaksi pembukuan toko _retail_ untuk kemudian dianalisa. Jadi berbeda dengan survey atau tipe riset yang mengumpulkan data _end-consumer_ melalui metode wawancara (_interview_), _retail audit_ mengumpulkan data dari langsung dari pembukuan toko (baik toko modern atau tradisional). Biasanya pihak _researcher_ bekerja sama dan membuat perjanjian dengan toko terpilih untuk bisa mendapat data tersebut selama periode waktu tertentu. Ada dua cara pengumpulan data yang lazim dilakukan saat _retail audit_:

1. _Researcher_ memberikan buku atau _worksheet_ yang harus diisi selama periode waktu tertentu oleh pihak toko. Hal ini bisa dilakukan untuk toko tradisional yang belum memiliki pembukuan transaksi. Salah satu keuntungan yang bisa didapatkan oleh pihak toko adalah bisa mempelajari pembukuan yang baik untuk usahanya.
1. _Researcher_ mencatat data transaksi dari pembukuan toko. Biasanya hal ini dilakukan untuk toko modern atau toko besar.

Semua transaksi dari toko kepada konsumen seperti:

1. Waktu transaksi,
1. SKU yang terjual, dan
1. Harga per SKU.

Menjadi data yang krusial. Namun tidak menutup kemungkinan ada data lain yang perlu diambil untuk menyelesaikan _research problem_.

Sebagai contoh, biasanya data yang didapatkan adalah sebagai berikut:

```{r,echo=FALSE}
library(tidyr)
rm(list=ls())
n_data = 15
n_trans = sample(1:4,n_data,replace=T)
id = 1:n_data
prod = paste0("Product ",1:10)
harga = sample(10:25,10) * 100000

df = data.frame(id_transaksi = id,product = NA,qty = NA,price = NA)

for(i in 1:n_data){
  id_prod = sample(1:10,n_trans[i])
  prod_s = prod[id_prod]
  harga_s = harga[id_prod]
  qty_s = sample(1:3,n_trans[i],replace = T)
  df$product[i] = paste(prod_s,collapse = ",")
  df$qty[i] = paste(qty_s,collapse = ",")
  df$price[i] = paste(harga_s,collapse = ",")
}

df = 
  df %>% 
  separate_rows(product,qty,price,sep = ",") %>% 
  mutate(qty = as.numeric(qty),
         price = as.numeric(price))

df %>% knitr::kable(align  = "c", caption = "Contoh Data Transaksi yang Dikumpulkan")
```

Beberapa analisa yang bisa dilakukan antara lain:

### _Market Share_

Inti dari analisa _market share_ adalah mencari tahu seberapa besar proporsi _sales value_ per produk, merek, atau kategori tertentu dari data. Sebagai contoh, dari data di atas saya akan menghitung _market share_ untuk semua produk yang terlibat.

Langkah pengerjaannya cukup mudah, yakni dengan menjumlahkan terlebih dahulu total _sales value_ per produk lalu menghitung proporsi (persentase).

```{r,echo=FALSE}
omset =
  df %>%
  mutate(omset = qty * price) %>%
  group_by(product) %>%
  summarise(total_sales_value = sum(omset)) %>%
  ungroup() %>% 
  mutate(market_share = total_sales_value / sum(total_sales_value) * 100,
         market_share = round(market_share,2),
         market_share = paste0(market_share,"%")) %>% 
  arrange(desc(total_sales_value))

total = omset$total_sales_value %>% sum() / 1000000 
total = total %>% round(2)

omset %>% knitr::kable(align="c")
```

### _Market Size_

Sedangkan _market size_ adalah menghitung seberapa besar pasar yang bisa diraih oleh suatu produk atau kategori produk tertentu. 

Sebagai contoh, jika semua produk yang ada di atas berada pada kategori yang sama (misalkan: minuman ringan), maka kita bisa hitung _market size_-nya adalah sebesar `r paste0("Rp ",total," jt")`

## _TV Campaign Research_

## _Brand Health_

## _Market Entry Research_

## _Service Audit: Mistery Shopping_
