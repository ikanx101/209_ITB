---
title: Optimization of E-Commerce Product Discount Portfolio Using Modified Dynamic
  Spiral Optimization Algorithm
author: "Mohammad Rizka Fadhli"
date: "| Department of Computational Science, Natural Sciences Faculty, \n| Institut
  Teknologi Bandung\n| Jalan Ganesha No. 10 Bandung, West Java, Indonesia\n| Corresponding
  author's email: 20921004@mahasiswa.itb.ac.id\n"
output:
  pdf_document:
    number_sections: no
    toc: no
...
linestretch: 1.25
colorlinks: yes
abstract: The development of the digital economy market in Indonesia is growing every
  year. As a result, marketplaces have to compete with each other to get consumers.
  abc.com is one of the pioneer marketplace in Indonesia has strategy to provide
  additional discounts on listed products. Product discount portfolio problem is a
  Binary Linear Programming (BLP) problem with a decision variable in the form of
  a binary number that states whether a product is eligible for a discount or not.
  With many decision variables involved, we developed metaheuristic approaches Spiral
  Dynamic Optimization Algorithm (SDOA) to solve the optimization problem. The SDOA
  solution is proven to be more optimal (generates higher revenur) than the existing
  abc.com solution by average $9.8 \%$.
header-includes:
- \usepackage{fancyhdr}
- \pagestyle{fancy}
...
Each portfolio contains 100 product candidates. abc.com has the following problem: choosing the right product from the portfolio to generate the maximal revenue with a budget limit of 5 million rupiahs.
...
1. The existing solution from abc.com.
...
In this study, the portfolio used is derived from the calculation of the price elasticity study previously conducted by abc.com. 
